...
% This script is the experiental script for Experiment 2 in Han et al.,
% Holistic Ensemble Perception (in prep)
% MAIN_Original.m
...
%%
Mean_Order = [3.8,4.6,5.2,5.8,6.4,7;4.6,5.2,5.8,6.4,7,8]; %6 "parts" to ensure that the emotional valence is nearly evenly distributed
...
for ite = 1:nprac
    ...
    DrawFormattedText(window,'+','center','center');
    Screen('DrawTextures',window,pointer_face(pic_num),[],rect_6);
    Screen('Flip',window);
    WaitSecs(Duration);
    Screen('Flip',window);
    WaitSecs(Waiting_Time);
...
for ite = 1:ntrial
    ...
    rect_num = randperm(6,TRIAL);
    rect_num = sort(rect_num); %Note that this line was added after the first 10 subjects finished the exp.
    rect_6 = RECT(:,rect_num);
    DrawFormattedText(window,'+','center','center');
    Screen('Flip',window);
    WaitSecs(Waiting_Time);
    ...
    cd DATA_Original  % The data is saved in every single loop, so that crashing will not cause too much loss
    save(filename);
    cd ..
end
