%% This is the main function of the program -- spatially distributed display
...
Version = 'Upright_Smaller'; %Version control
load Image_Pilot
...
MACHINE = 0.8; %Adjustment of Stimuli
...
xStart = window_w * 0.37;
xEnd = window_w * 0.63;
yStart = window_h * 0.37;
yEnd = window_h * 0.63;
nRows = 2;
nCols = 3;
...
for ite = 1:ntrial
    ...
    rect_num = randperm(6,TRIAL);
    rect_num = sort(rect_num); %to ensure that the locations are distributed sequentially
    rect_6 = RECT(:,rect_num);
    DrawFormattedText(window,'+','center','center');
    Screen('Flip',window);
    WaitSecs(Waiting_Time);
    ...
    cd DATA_Original_Small  % The data is saved in every single loop, so that crashing will not cause too much loss
    save(filename);
    cd ..
end
