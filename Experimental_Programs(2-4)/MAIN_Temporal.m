%% This is the main function of the program -- sequential display
% On each trial, 1 ~ 6 faces are randomly selected from the stimulus set.
% These faces are simultaneously displayed for 300 msec in total.
...
%% Defining the Grid
img_height = 256 * .8; %MACHINE;
img_width = 256 * .8; %MACHINE;
xStart = window_w * 0.60; % original 25
xEnd = window_w * 0.40; %original 75
yStart = window_h * 0.3;
yEnd = window_h * 0.7;
nRows = 2;
nCols = 3;
...
for ite = 1:nprac
    ...
    for Disp = 1:TRIAL
        ...
        DrawFormattedText(window,'+','center',y_center - Move);
        Screen('DrawTextures',window,pointer_face(pic_num(Disp)),[],rect_6(:,Disp));
        Screen('Flip',window);
    ...
    Get_Response; %Slider bar function
end
...
for ite = 1:ntrial
    ...
    cd DATA_Temporal  % The data is saved in every single loop, so that crashing will not cause too much loss
    save(filename);
    cd ..
end
