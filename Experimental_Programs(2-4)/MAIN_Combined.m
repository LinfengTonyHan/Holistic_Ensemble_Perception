%% This is the main function of the program -- spatially distributed display, upright + inverted
...
% Upright viewing orientations and inverted viewing orientations are interleaved
...
UpDown_Order = Shuffle([zeros(1,ntrial/2),ones(1,ntrial/2)]);

%%
Mean_Order = [3.8,4.6,5.2,5.8,6.4,7;4.6,5.2,5.8,6.4,7,8]; %6 "parts" (uniform distribution)
...
for ite = 1:nprac
    ...
    DrawFormattedText(window,'+','center','center');
    Screen('DrawTextures',window,pointer_face(pic_num),[],rect_6,UpDown_Order(ite)*180);
    Screen('Flip',window);
    WaitSecs(Duration);
    Screen('Flip',window);
    WaitSecs(Waiting_Time);
...
for ite = 1:ntrial
    ...
    cd DATA_Combined  % The data is saved in every single loop, so that crashing will not cause too much loss
    save(filename);
    cd ..
end
